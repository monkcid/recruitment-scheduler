// Vercel Serverless Function - Secure Smartsheet API Proxy
// This keeps the token safe and handles all Smartsheet API calls
...
async function updateRow(token, sheetId, rowData, res) {
  const response = await fetch(`https://api.smartsheet.com/2.0/sheets/${sheetId}/rows/${rowData.id}`, {
    method: 'PUT',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
